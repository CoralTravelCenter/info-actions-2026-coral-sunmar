...
`filters` (нормализация канонического `filters` или legacy `filter`) и
`analytics.bonusImpression`.

Канонические поля — `id`, `filters: string[]`, `legal`. Legacy-поля `filter` и `ligal`
...
только очищенное название акции; внешний скрипт использует его как значение параметра
Метрики, не связывая Vue-блок с конкретным провайдером.
