...
src/
  ...
  utils/configWarnings.ts       — проверки конфига для предупреждений в интерфейсе
```
...
`filters` (разбор `filter`) и `analytics.bonusImpression`.

### Аналитика кликов

Vue-блок не отправляет click-цели самостоятельно. Перед стандартным переходом он
синхронно публикует на `window` событие `promotion-card:click`. Формат `detail`
версионирован и описан в `analytics/promotionEvents.ts`; внешний скрипт может направить
его в Метрику, `dataLayer` или другую систему, не связывая виджет с конкретным провайдером.

```js
window.addEventListener("promotion-card:click", event => {
  const {version, promotion, context} = event.detail;
});
```

Отсутствующие `entry_point` больше не влияют на работу карточек и не создают
предупреждений. Техническое уведомление остаётся для действительно невосстановимых
ошибок: отсутствующих `name`/`visual` и дублирующихся `id`.
